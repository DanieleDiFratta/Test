package com.mindsapp.test.model;

import android.app.Activity;
import android.content.SharedPreferences;
import android.support.annotation.MainThread;

import com.mindsapp.test.MainActivity;
import com.mindsapp.test.RSSICollectorActivity;

import java.math.BigDecimal;
import java.math.BigInteger;
import java.math.MathContext;
import java.math.RoundingMode;
import java.util.ArrayList;
import java.util.List;
import java.util.Map;

/**
 * Created by Daniele on 22/05/2016.
 * Calculates Threshold giving RSSI values
 */
public class ThresholdCalculator {

    private static final double PFA = 0.0001;
    private List<Integer> testVariables;

    public ThresholdCalculator() {
        testVariables = new ArrayList<>();
    }

    public double calculateThres(Map<String, List<Integer>> rssiValues) throws Exception {
        double variance,mean,threshold;
        createTestValues(rssiValues);
        mean = calculateArtMean(this.testVariables);
        variance = calculateVariance(this.testVariables,mean);
        threshold = mean + (2.0*Math.sqrt(variance)) * invErf(1.0 - 2.0 * PFA);
        return threshold;
    }

    private double calculateVariance(List<Integer> testVariables, double mean) {
        double variance = 0;
        int numElements = testVariables.size();

        for(double d : testVariables)
            variance += Math.pow(d - mean, 2);

        return variance / (double)numElements;
    }

    public void createTestValues(Map<String, List<Integer>> rssiValues) {
        List<Integer> maxValues = getMaxValues(rssiValues);
        List<Integer> partialValues = new ArrayList<>();
        int count = 0;
        int testVariablesCount = maxValues.size() / MainActivity.getContextofApplication()
                .getSharedPreferences(RSSICollectorActivity.RSSI_COLLECTOR_PREFERENCES, Activity.MODE_PRIVATE)
                .getInt(RSSICollectorActivity.NUMBER_OF_TESTS, 0);
        for (Integer rssi :
                maxValues) {
            if(count<testVariablesCount){
                partialValues.add(rssi);
                count++;
            }
            else{
                calculateTestVariable(partialValues);
                partialValues.clear();
                partialValues.add(rssi);
                count = 1;
            }
        }
        //calculateTestVariable(partialValues);
    }

    private void calculateTestVariable(List<Integer> partialValues) {
        int artMean = calculateArtMean(partialValues);
        int wheMean = calculateWheMean(partialValues);
        this.testVariables.add(wheMean-artMean);
    }

    private int calculateWheMean(List<Integer> partialValues) {
        int num=0,den=0,whe=1;
        for (Integer rssi :
                partialValues) {
            num += whe * rssi;
            den += whe;
            whe++;
        }
        return num/den;
    }

    private int calculateArtMean(List<Integer> partialValues) {
        int sum = 0;
        for (Integer rssi :
                partialValues) {
            sum += rssi;
        }
        return sum/partialValues.size();
    }

    private List<Integer> getMaxValues(Map<String, List<Integer>> rssiValues) {
        int max = 0;
        List<Integer> result = null;
        for (List<Integer> list :
                rssiValues.values()) {
            max = list.size() > max ? list.size() : max;
            result = list;
        }
        return result;
    }

    /*calcola la funzione di errore inversa*/

    public static double invErf(double d) throws Exception {

        if (Math.abs(d)>1) {

            throw new Exception ("Allowed values for argument in [-1,1]");

        }

        if (Math.abs(d) == 1) {

            return (d==-1 ? Double.NEGATIVE_INFINITY :

                    Double.POSITIVE_INFINITY);

        }

        else {

            if (d==0) {

                return 0;

            }

            BigDecimal bd = new BigDecimal(0, MathContext.UNLIMITED);

            BigDecimal x = new

                    BigDecimal(d*Math.sqrt(Math.PI)/2,MathContext.UNLIMITED);

            //System.out.println(x);

            String[] A092676 = {"1", "1", "7", "127", "4369", "34807",

                    "20036983", "2280356863",

                    "49020204823", "65967241200001",

                    "15773461423793767",

                    "655889589032992201",

                    "94020690191035873697", "655782249799531714375489",

                    "44737200694996264619809969",

                    "10129509912509255673830968079", "108026349476762041127839800617281",

                    "10954814567103825758202995557819063",

                    "61154674195324330125295778531172438727",

                    "54441029530574028687402753586278549396607",

                    "452015832786609665624579410056180824562551",

                    "2551405765475004343830620568825540664310892263",



                    "70358041406630998834159902148730577164631303295543",

                    "775752883029173334450858052496704319194646607263417",



                    "132034545522738294934559794712527229683368402215775110881"};



            String[] A092677 = {"1", "3", "30", "630", "22680", "178200",

                    "97297200", "10216206000",

                    "198486288000", "237588086736000",

                    "49893498214560000",

                    "1803293578326240000",

                    "222759794969712000000","1329207696584271504000000",

                    "77094046401887747232000000",

                    "14761242414008506896480000000", "132496911908140357902804480000000",

                    "11262237512191930421738380800000000",

                    "52504551281838779626144331289600000000",

                    "38905872499842535702972949485593600000000",

                    "268090886133368733415443853598208000000000",

                    "1252532276140582782027102181569679872000000000",

                    "28520159927721069946757116674341610685440000000000",



                    "259078091444256105986928093487086396226560000000000",

                    "36256424429074976496234665114956818633529712640000000000"};



            for (int i = 0; i < A092676.length; i++) {

                BigDecimal num = new BigDecimal(new BigInteger(A092676[i]),

                        50);

                BigDecimal den = new BigDecimal(new BigInteger(A092677[i]),

                        50);

                BigDecimal coeff = num.divide(den, RoundingMode.HALF_UP);

                //System.out.println(coeff);

                BigDecimal xBD = x.pow(i*2+1, MathContext.UNLIMITED);



                bd = bd.add(xBD.multiply(coeff, MathContext.UNLIMITED));



            }

            return bd.doubleValue();

        }

    }
}
